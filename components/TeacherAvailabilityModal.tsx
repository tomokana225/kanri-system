  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === 'single') {
        ...
        await addRecurringAvailabilities(user.id, daysOfWeek, recurringTimes, new Date(), new Date(endDate));
      }
      onSaveSuccess();
      onClose();
    } catch (e: any) {
    ...
    <Modal title="空き時間の登録" onClose={onClose}>
      <div className="space-y-4">
        ...
        <div className="flex justify-end space-x-2 pt-4">
          ...
          <button onClick={handleSubmit} disabled={loading} className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 disabled:opacity-50">
            {loading ? '保存中...' : '保存'}
          </button>
        </div>
      </div>
